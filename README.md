...
const api = require('@tomassabol/lambda-api')();
...
```
npm i @tomassabol/lambda-api --save
```
...
Require the `@tomassabol/lambda-api` module into your Lambda handler script and instantiate it. You can initialize the API with the following options:
...
const api = require('@tomassabol/lambda-api')({ version: 'v1.0', base: 'v1' });
```
...
```javascript
const api = require('@tomassabol/lambda-api')({ callback: 'cb' });
...
```javascript
const api = require('@tomassabol/lambda-api')({ logger: true });
...
const api = require('@tomassabol/lambda-api')({ logger: { level: 'warn' } });
...
If you are using persistent connections in your function routes (such as AWS RDS or Elasticache), be sure to set `context.callbackWaitsForEmptyEventLoop = false;` in your main handler. This will allow the freezing of connections and will prevent Lambda from hanging on open connections.
...
import createAPI from '@tomassabol/lambda-api';
...
Contributions, ideas and bug reports are welcome and greatly appreciated. Please add [issues](https://github.com/tomassabol/lambda-api/issues) for suggestions and bug reports or create a pull request.
